...
# S6-deploy 배포 완료 알람 채널 ID (#ops-배포, 공개). 비면 발송 안 함(no-op).
...
gcloud run deploy "$SERVICE" \
  ...
  --set-env-vars "KIS_TOKEN_CACHE=/mnt/token/.kis-token.json,TOSS_TOKEN_CACHE=/mnt/token/.toss-token.json,CACHE_DIR=/mnt/cache,DATA_DIR=/mnt/data,SLACK_OPS_CHANNEL=C0BA29NTQUF,SLACK_BRIEFING_CHANNEL=C0BABCPKLCB,SLACK_SIGNAL_CHANNEL=$SLACK_SIGNAL_CHANNEL,SLACK_AI_COMMENT_CHANNEL=$SLACK_AI_COMMENT_CHANNEL,SLACK_EVENT_CHANNEL=$SLACK_EVENT_CHANNEL,SLACK_DEPLOY_CHANNEL=$SLACK_DEPLOY_CHANNEL,SLACK_COST_CHANNEL=$SLACK_COST_CHANNEL,GCP_PROJECT_ID=$PROJECT,TASKS_LOCATION=$REGION,TASKS_QUEUE=$TASKS_QUEUE" \
  --set-secrets "KIS_APP_KEY=KIS_APP_KEY:latest,KIS_APP_SECRET=KIS_APP_SECRET:latest,TOSS_CLIENT_ID=TOSS_CLIENT_ID:latest,TOSS_CLIENT_SECRET=TOSS_CLIENT_SECRET:latest,NAVER_CLIENT_ID=NAVER_CLIENT_ID:latest,NAVER_CLIENT_SECRET=NAVER_CLIENT_SECRET:latest,ANTHROPIC_API_KEY=ANTHROPIC_API_KEY:latest,DART_API_KEY=DART_API_KEY:latest,ECOS_API_KEY=ECOS_API_KEY:latest,EDGE_API_TOKEN=EDGE_API_TOKEN:latest,SLACK_BOT_TOKEN=SLACK_BOT_TOKEN:latest,SLACK_SIGNING_SECRET=SLACK_SIGNING_SECRET:latest"
