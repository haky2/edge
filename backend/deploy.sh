...
# ⚠️ 토스(TOSS_CLIENT_ID/SECRET) 키는 의도적으로 미주입 — 토스 OpenAPI는 토큰 발급을 등록된 IP만
#    허용하는데(access_denied: IP not allowed) Cloud Run 송신 IP는 동적이라 허용목록에 못 넣는다.
#    키가 없으면 TossClient가 네트워크 시도 없이 즉시 no-op(warnings=[], calendar/limits=숨김) → 운영은 한투만.
#    재활성화: Cloud NAT 고정 송신 IP 확보 → 토스 콘솔 허용목록 등록 → 위 --set-secrets 에 TOSS_CLIENT_ID/SECRET 추가.
#    (Secret Manager에 TOSS_CLIENT_ID/SECRET 시크릿 자체는 등록돼 있어 추가만 하면 됨)
