    ...
    companion object {
        ...
        private val DEFENSIVE_PROMPT = """
            ...
            9. "방향 예측 자기 성적표" 항목이 있으면(없으면 이 규칙 전체를 무시) — 같은 지표들로 만든 방향 예측이
               ...
               또한 무작위 기준(약 33%)을 웃돈다는 이유로 예측력이 검증된 듯 서술하지도 마라 — 한 방향으로
               쏠린 장에서는 그 방향만 찍어도 40%대 중후반이 나온다(기저율).
        ...
        private val AGGRESSIVE_PROMPT = """
            ...
            9. "방향 예측 자기 성적표" 항목이 있으면(없으면 이 규칙 전체를 무시) — 같은 지표들로 만든 방향 예측이
               ...
               무작위 기준(33%)을 웃돈다고 예측력이 검증된 듯 말하지도 마라 — 쏠린 장에선 한 방향
               찍기만으로도 40%대가 나온다(기저율).
