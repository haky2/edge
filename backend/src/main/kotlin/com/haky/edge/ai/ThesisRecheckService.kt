...
class ThesisRecheckService(
    ...
    companion object {
        ...
        private val SYSTEM_PROMPT = """
            ...
            네 판정 중 "약화"와 "무효"만 사용자에게 알림으로 전달된다 — 근거 없는 경보는 그 자체로 해악이고,
            근거 있는 반론을 삼키는 것도 이 점검의 존재 이유를 없앤다. 두 방향 모두에서 정확해야 한다.

            반드시 아래 JSON 객체 하나만 출력하라. 코드펜스(```)·설명·서두 텍스트 금지.
            {
              "verdict": "유효" | "약화" | "무효" | "판단불가",
              "changedFact": "논지에 영향을 준 오늘의 사실 한 줄(사실 데이터·오늘의 변화에서만. 없으면 생략)",
              "reason": "왜 그 판정인지 1~2문장(한국어). 매매 지시(사라·팔라) 금지 — 판정과 근거만. 이 지침의 규칙 번호를 언급하지 마라"
            }
            ...
            5. "약화"=논지의 핵심 근거가 부분적으로 흔들림. "무효"=핵심 전제가 사실 데이터에서 소멸·반증됨. 애매하면 낮은 강도로 판정하라(과잉 경보 금지). 논지가 그 변화를 이미 전제·감수하고 있으면(예: "단기 부진은 감수") 그 변화만으로 약화로 판정하지 마라 — 논지가 예상하지 못한 변화만이 강도를 만든다.
