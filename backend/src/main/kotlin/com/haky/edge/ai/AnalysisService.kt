...
class AnalysisService(
    ...
    companion object {
        ...
        private val SYSTEM_PROMPT = """
            ...
            2. 다음 흐름으로 자연스럽게 이어지는 4~6문단으로 써라:
               ① 최근 주가가 왜 이렇게 움직였나 — 뉴스와 가격 흐름을 연결해 "무슨 일이 있었는지" 쉽게 설명.
               ② 그 움직임이 일시적 기대인지 실제 실적 변화인지 — "회사 재무" 섹션이 있으면 매출·이익 추세와 비교.
               ③ PER/PBR이 있으면 쉬운 말로 풀어라: "이 주가는 회사 1년 순이익의 N배를 내는 셈인데, 같은 업종 내 다른 종목과 비교하거나 과거 평균과 비교했을 때 높은 편인지/낮은 편인지" 식으로.
               ④ 컨센서스 목표주가가 있으면 현재가와 어떻게 다른지, 그 의미가 무엇인지 짧게 설명.
               ⑤ 지금 이 종목을 어떻게 봐야 하는지 — 수급 흐름(누가 사고 파는 추세인지)과 앞의 내용을 모아 정리.
            ...
            6. 불릿 목록 금지. 이야기처럼 흐르는 문단으로.
            7. 뉴스는 종목과 무관한 것이 섞일 수 있다. 관련 있어 보이는 것만 쓰고 억지로 연결하지 마라.
            8. "내 포지션" 섹션이 있으면 평단가 기준 현재 손익과 목표가까지 남은 거리를 코멘트에 자연스럽게 녹여준다.
        """.trimIndent()
    }
}
