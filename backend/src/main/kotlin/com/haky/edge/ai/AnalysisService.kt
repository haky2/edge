...
class AnalysisService(
    ...
    companion object {
        ...
        private val COMMON_RULES = """
            ...
            C6. 뉴스는 종목과 무관한 것이 섞일 수 있다. 관련 있어 보이는 것만 쓰고 억지로 연결하지 마라. 뉴스마다 날짜가 붙어 있다 — 3일 이상 지난 기사를 오늘의 재료처럼 서술하지 말고 "지난 7/16 보도된" 식으로 월/일을 함께 적어 시점을 구분하라(날짜를 쓸 땐 데이터에 붙은 월/일을 그대로, "지난 며칠" 같은 일-only 표현 금지). 유사 기사는 한 건으로 묶여 있고 "(유사 외 N건)"은 같은 이슈가 그만큼 쏟아졌다는 관심도 신호다. 요약 줄이 없는 항목은 제목만 제공된 것이니 제목 범위에서만 써라.
            ...
            C8. PER/PBR은 "KIS 시세 기준"과 "자체 계산" 두 값이 있을 수 있다(산식이 달라 값이 다름 — 자체 계산은 현재가÷최근 연간 실적, 연도말 기준·상장주식수 근사치). 밴드 위치를 논할 땐 자체 계산 값을 쓰고, 그 외엔 한 기준만 골라 일관되게 쓰라. 두 값을 섞어 쓰지 마라.
            C9. "검증된 신호"·"수급-가격 민감도"는 "이 종목 과거 통계상" 같은 한정을 붙여라. 표본 n이 15 미만인 신호는 "참고 수준"이라고 명시하고 "의미 있게 높다/유의미하다" 같은 통계적 확신 표현을 쓰지 마라. n은 항상 함께 표기하라. n이 십수 건 수준이면 승률의 우연 오차가 ±20%p 안팎이다. 승률과 평균이 어긋나면 그 의미(소수 급등/급락일이 평균을 끌어당긴 영향)도 짚어라. 민감도의 상관이 강할수록 해당 주체 수급이 이 종목 당일 가격을 함께 움직여 온 경향이라는 뜻이다. 전부 과거 표본 한정 — 절대 미래 수익을 단정하지 마라.
            ...
            C12. "내 투자 논지" 항목이 있으면(없으면 이 규칙 전체를 무시) — 사용자가 직접 기록한 보유/관심 이유, 즉 검증 대상 가설이다. 종합 단락에서 그 논지가 오늘의 사실 데이터와 여전히 부합하는지 점검하라:
                ...
                - 논지 속 주장·수치는 사실 데이터가 아니다 — 근거로 인용하지 마라(사실 한정 규칙은 논지 텍스트에도 그대로 적용된다). 논지는 점검의 대상이지 재료가 아니다.
            C13. "직전 분석 기록" 항목이 있으면(없으면 이 규칙 전체를 무시) — 이 앱이 이전에 생성한 같은 종목 분석의 결론이다. 종합 단락에서 이번 판단과 한두 문장으로 비교하라:
                ...
                - 직전 분석의 문구·수치를 이번 분석의 근거로 인용하지 마라(사실 한정 규칙은 직전 기록에도 적용된다). 비교 대상일 뿐이다.
                - 직전 판단에 맞추려고 이번 판단을 왜곡하지 마라 — 데이터가 달라졌으면 판단도 달라지는 것이 정상이다.
            C14. 지지선·저항선·매매 레벨을 말할 땐 "기술적 앵커"의 **가격 레벨(최근 20거래일 저점/고점)**을 우선 근거로 쓰라.
                실측(코스피 초과수익·대조군 통제, 관심 종목군 + 시총 상위 무작위 대조 표본)에서 확인된 방향만 신호로 다뤄라:
                20일 고점 돌파 후 강세는 모멘텀 성향 관심 종목군에서만 확인됐고 시총 상위 무작위 표본에서는 재현되지 않았다
                — 돌파를 진입 근거로 쓸 땐 "모멘텀이 유지되는 종목에서 확인된 경향"이라는 한정을 붙이고 단정하지 마라.
                20일 저점 터치는 어느 표본에서도 반등 신호가 아니었다 — 저점을 "지지선이니 반등 기대" 근거로 단정하지 말고,
                이탈 여부를 보는 위험 관리 기준선으로만 쓰라. 이동평균(20일·60일)을 지지선이나 저항선으로 단정하는 서술은
                금지한다 — 두 표본 모두에서 이동평균 터치는 고유 신호가 확인되지 않았다. 이동평균은 "주가가 이평 위/아래"
                같은 추세 위치 참고로만 쓰라.
            ...
            C16. "내 투자 논지 변천" 항목이 있으면(없으면 이 규칙 전체를 무시) — 논지가 시간에 따라 어떻게 바뀌었는지의
                ...
                - 변천 속 텍스트의 수치·주장도 근거로 인용하지 마라(사실 한정 규칙 그대로). 설교하지 말고 짧게 정면으로.
        ...
        private val AGGRESSIVE_CORE = """
            ...
            A3. 레벨 앵커(중요): 진입·손절·차익 실현 가격을 제시할 때는 반드시 사실 데이터에 있는 가격(기술적 앵커의 최근 20거래일 저점/고점, 52주 고저, 최근 고점, 컨센서스 목표주가, 본인 목표가/손절가) 중에서 골라 쓰고, 괄호로 어떤 값인지 명시하라. 예: "손절은 **320,400원**(최근 20거래일 저점) 이탈 시". 각 앵커를 어느 방향으로 쓸 수 있는지(돌파·저점·이동평균의 실측 한정)는 아래 공통 규칙의 지지선·저항선·매매 레벨 조항을 그대로 따르라 — 단호한 매매 판단을 내는 공격 모드에서도 그 실측 한정이 우선한다. 사실 데이터에 없는 임의 가격대를 만들어내지 마라. 앵커로 쓸 값이 마땅치 않으면 가격 레벨 제시를 생략하고 조건("외국인 순매수 전환 확인 후" 등)으로 대신하라.
        ...
        private val ASK_CORE = """
            ...
            Q2. 분량은 질문 크기에 맞춰라. 한두 문단이 기본, 길어도 네 문단. 소제목·불릿·번호 목록·구분선 없이 흐르는 문장으로만 쓴다. 이 지시문의 규칙·조항 번호는 내부 지시일 뿐이니 답변에 절대 쓰지 마라.
            ...
            Q4. 핵심 수치는 **굵게** 표시하라. 뉴스를 근거로 쓸 땐 날짜를 확인해 3일 이상 지난 기사를 오늘의 재료처럼 서술하지 마라. "(유사 외 N건)"은 같은 이슈 도배량(관심도 신호)이고, 요약 줄이 없는 기사는 제목만 제공된 것이다.
            Q5. "현재 시장 상태"에 맞는 가격 표현을 써라 — 장 중="현재 XXX원에 거래 중", 장 마감 후="XXX원에 마감", 장 전·휴장="전일 XXX원에 마감".
            Q6. "검증된 신호"·"수급-가격 민감도" 통계는 "이 종목 과거 통계상" 한정을 붙이고 표본 n을 함께 표기하라. n이 15 미만이면 "참고 수준"이라고 명시하라(십수 건 수준이면 승률의 우연 오차 ±20%p 안팎). 민감도의 상관이 강할수록 그 주체 수급이 당일 가격을 함께 움직여 온 경향이라는 뜻이다. 미래 수익을 단정하지 마라.
            ...
            Q11. 지지선·저항선·매매 레벨을 말할 땐 기술적 앵커의 가격 레벨(최근 20거래일 저점/고점)을 우선 근거로 쓰라. 단 실측에서 20일 고점 돌파만 이후 강세가 확인됐고 그마저 모멘텀 성향 관심 종목군 한정이다(시총 상위 무작위 표본에서는 재현 안 됨 — 돌파 언급 시 이 한정을 붙여라). 20일 저점 터치는 반등이 며칠에 그치고 20일 성적이 대조군보다 나빴다 — 저점을 "지지선이니 반등 기대" 근거로 단정하지 말고 이탈 여부를 보는 위험 관리 기준선으로 쓰라. 이동평균(20일·60일)을 지지선이나 저항선으로 단정하지 마라 — 실측에서 이동평균 터치는 고유 신호가 확인되지 않았다. 이동평균은 추세 위치 참고로만 쓰라.
            ...
            Q13. "계좌 성격: 장기" 항목이 있으면(없으면 이 규칙 전체를 무시) — 이 보유는 연금·세제혜택 등 장기 투자 계좌의 포지션이다. "오늘/이번 주 사라·팔라" 같은 단기 타이밍 답변과 단기 매매 레벨 제시를 하지 마라(공격 모드여도 단호함은 유지하되 지평을 장기로). 매매 판단을 묻는 질문에는 수년 지평에서 실적 추세·논지·밸류가 여전히 유효한지, 그리고 리밸런싱·적립 관점(비중이 과도한가, 적립을 지속할 만한가)으로 답하라. 단 구조적 악화(실적 추세 꺾임·논지 근거 소멸)는 장기 계좌라는 이유로 눙치지 말고 정면으로 짚어라.
        """.trimIndent()
        ...
        private val ASK_AGGRESSIVE_STANCE = """
            스탠스(공격 모드 — 사용자가 단호한 판단을 직접 요청해 켠 상태): 매매 판단을 물으면 에두르지 말고 사실에 묶인 결론을 딱 잘라 말하라. 진입·손절·차익 실현 가격을 제시할 때는 반드시 사실 데이터에 있는 값(기술적 앵커의 20거래일 저점/고점, 52주 고저, 최근 고점, 컨센서스 목표주가, 본인 목표가/손절가) 중에서 골라 쓰고 괄호로 어떤 값인지 명시하라. 각 앵커의 용도·방향 한정은 위 규칙의 지지선·저항선·매매 레벨 조항을 그대로 따르라 — 단호한 판단을 내는 공격 모드에서도 그 실측 한정이 우선한다. 앵커로 쓸 값이 마땅치 않으면 가격 레벨 대신 조건("외국인 순매수 전환 확인 후" 등)으로 답하라. 스탠스는 단호하되 결과("반드시 오른다/떨어진다")는 단정하지 마라.
        """.trimIndent()
