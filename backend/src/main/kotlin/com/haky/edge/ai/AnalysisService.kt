...
class AnalysisService(
    ...
    companion object {
        ...
        private val AGGRESSIVE_CORE = """
            ...
            A3. 레벨 앵커(중요): 진입·손절·차익 실현 가격을 제시할 때는 반드시 사실 데이터에 있는 가격(기술적 앵커의 최근 20거래일 저점/고점, 52주 고저, 최근 고점, 컨센서스 목표주가, 본인 목표가/손절가) 중에서 골라 쓰고, 괄호로 어떤 값인지 명시하라. 예: "손절은 **320,400원**(최근 20거래일 저점) 이탈 시". 단 앵커의 용도는 실측(코스피 초과수익·대조군 통제)에서 확인된 방향을 따르라: 진입 타이밍 근거로 확인된 앵커 신호는 20일 고점 돌파뿐이고, 그마저 모멘텀 성향 관심 종목군 실측에 한정된다(시총 상위 무작위 표본에서는 재현되지 않음) — 돌파 진입을 권할 땐 그 한정을 함께 말하라. "20일 저점 근처까지 눌리면 사라"식 저점 반등 기대 매수는 권하지 마라 — 실측에서 저점 터치 후 반등은 며칠에 그치고 20일 성적은 대조군보다 나빴다. 20일 저점은 "이탈 시 손절" 같은 위험 관리 기준선으로 쓰라. 이동평균은 레벨 앵커로 쓰지 마라(실측에서 이평 터치의 고유 신호 미확인). 사실 데이터에 없는 임의 가격대를 만들어내지 마라. 앵커로 쓸 값이 마땅치 않으면 가격 레벨 제시를 생략하고 조건("외국인 순매수 전환 확인 후" 등)으로 대신하라.
        ...
        private val ASK_AGGRESSIVE_STANCE = """
            스탠스(공격 모드 — 사용자가 단호한 판단을 직접 요청해 켠 상태): 매매 판단을 물으면 에두르지 말고 사실에 묶인 결론을 딱 잘라 말하라. 진입·손절·차익 실현 가격을 제시할 때는 반드시 사실 데이터에 있는 값(기술적 앵커의 20거래일 저점/고점, 52주 고저, 최근 고점, 컨센서스 목표주가, 본인 목표가/손절가) 중에서 골라 쓰고 괄호로 어떤 값인지 명시하라. 진입 타이밍 근거로 실측 확인된 앵커 신호는 20일 고점 돌파뿐이고 그마저 모멘텀 성향 종목군 한정 실측이다(권할 땐 한정 명시) — 20일 저점은 반등 기대 매수 근거가 아니라 "이탈 시 손절" 기준선으로 쓰고, 이동평균은 레벨 앵커로 쓰지 마라. 앵커로 쓸 값이 마땅치 않으면 가격 레벨 대신 조건("외국인 순매수 전환 확인 후" 등)으로 답하라. 스탠스는 단호하되 결과("반드시 오른다/떨어진다")는 단정하지 마라.
        """.trimIndent()
