...
class SlackFormatTest {
    ...
    @Test fun `볼드 뒤 여는 괄호 → 평문`() {
        // **9,650원**(-2.28%) → 9,650원(-2.28%) : Slack은 *값*( 를 볼드로 렌더 못 함(별표 노출 방지)
        assertEquals("9,650원(-2.28%)에", SlackFormat.sanitize("**9,650원**(-2.28%)에"))
    }

    @Test fun `볼드 뒤 여는 대괄호 → 평문`() {
        assertEquals("삼성전자[반도체]", SlackFormat.sanitize("**삼성전자**[반도체]"))
    }

    @Test fun `볼드 앞 닫는 괄호 → 평문`() {
        // (참고)**강세** → (참고)강세 : 여는 별표 앞이 닫는 괄호면 페어링이 깨진다
        assertEquals("(참고)강세 지속", SlackFormat.sanitize("(참고)**강세** 지속"))
    }

    @Test fun `괄호로 감싼 볼드는 유지`() {
        // (**강세**) → (*강세*) : 여는 별표 앞이 여는 괄호·닫는 별표 뒤가 닫는 괄호는 정상 렌더
        assertEquals("(*강세*)", SlackFormat.sanitize("(**강세**)"))
    }
