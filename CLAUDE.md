...
- **종목 상세 "지표 해석" (2층 구조)** — 상세 화면(StockDetailView)에 붙인다:
  - ① **계산 기반 (LLM 없이, 즉시)**: 52주 범위 내 현재 위치(%), 고점/저점 대비 낙폭, 거래량 평소 대비 수준 등 + 각 지표가 *뭘 의미하는지* 짧은 설명. "이 숫자를 어떻게 봐야 하나"의 1차 가이드.
    - **수급 흐름 요약(코드로)**: "외국인 N일 연속 순매도(누적 -X주), 기관 N일 연속 순매수" 같은 연속일·누적량 패턴을 즉시 한 줄로 → 수급 카드 위에 얹는다(LLM 없이, 무료).
  - ② **Claude 종합 코멘트 (① 위에 얹음)**: "지금 이 종목 어떻게 봐야 하나"를 **내 포지션 기준**으로 풀어줌. 원칙대로 *계산값 먼저 → Claude는 해석만*.
    - **수급 → 내 포지션 영향 해석**: "이 수급 흐름이 내 평단 대비 지금 어떤 의미인가"를 ①의 요약값을 입력으로 Claude가 해석(참고용 디스클레이머).
