...
- **Slack 연동** — 운영 오류·아침 브리핑·신호 알림·AI 코멘트 아카이브·이벤트 리마인더 + `/edge 종목명` 슬래시 명령.
...
이렇게 두면 ① 한투 계좌 없이 앱만 설치하면 되고 ② 키가 클라이언트에 노출되지 않으며 ③ 시세·분석을 공유 캐시로 묶어 비용을 1배 수준으로 누릅니다.
...
iOS·Android 풀 패리티 완성, 백엔드 Cloud Run 배포 중. 핵심 기능은 마무리 단계예요.

이 프로젝트는 **Claude로 코딩하고(Claude Code), AI 분석 기능도 Claude API로 구현**했습니다. 백엔드는 **Google Cloud Platform** 위에서 운영해요 — Cloud Run(서버)·Cloud Scheduler(아침 브리핑·신호 스캔 등 정기 잡)·Cloud Tasks(비동기 작업)·Secret Manager(키 보관)·Cloud Storage(캐시·토큰 영속화).
