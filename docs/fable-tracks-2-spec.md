...
- [x] 2a Analog 캘리브레이션 실측 (2026-07-14) — **캘리브레이션 실패**: 885표본, 버킷 단조성 없음(<45→54~59%, >60→57~59%)·Spearman 0.057·부호 일치 51%=나이브 동률·평균 매칭 7.8건. 교정 = CAVEAT_BASE에 실측 병기("승률 수치의 예측력 확인 안 됨 — 범위 참고용"). 정본 docs/discovery-analog-validation-2026-07.md
- [x] 2b Discovery 3신호 실측 (2026-07-14) — 저점반등 지지(+1.23%p·승률 +9.5%p)·상대모멘텀 지지(약, 그리드 {3,5,7} 전부 지지 → 컷 유지)·**신고가근접 반증(−1.32%p·−4.0%p, n=642) → 신호 제거**(decisions #15, 회귀 테스트). 제거 후 가격 교집합 +0.58%p·승률 +9.6%p 회복. 운영 실후보 0건(7/10~13 — 9월 재점검)
- [x] 3a MoodLog 가중치 실측 (2026-07-14) — /moodweight-validation, 483거래일·홀드아웃 분리. **7/8 SUPPORTED**(미국 지수 6종 ρ 0.27~0.34, dxy 약지지, usdkrw만 INCONCLUSIVE). 현행 43.9%=라이브 43% 재현 일치. 다수 클래스(BULLISH) 48.2% > 현행 — 기저율 함정 확인. 예측 중립 쏠림(43.7% vs 실제 20.3%) 기록. 정본 docs/moodweight-validation-2026-07.md
- [x] 3b 교정 (2026-07-14) — **현행 유지**(usdkrw 제거 후보가 홀드아웃 +0.4%p — 채택 기준 +2%p 미달, decisions #16). 가중치 불변이라 성적표 연속성 처리 불요. 규칙 9에 다수 클래스 기저율 교훈 보강(방어·공격). 선물 3종·임계 ±0.5는 9월 재론
