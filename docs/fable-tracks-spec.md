...
- [x] C1 백엔드 (2026-07-10) — GET /deep-research/{code}, DeepResearchService **2단계 구조**(①웹검색 수집=노트만·Sonnet ②합성 리포트=complete()·DEEP_RESEARCH 트리거 기본 Opus — completeWithWebSearch의 중간 서술 오염 회피, EventSync 패턴 확대). NumberGuard 미적용(웹 수치는 구조적 오탐) 대신 프롬프트 D1~D4 출처 규율(2층 수치 출처·시점 병기 의무, 상충 병기, 내부 용어 금지, 면책 문구 금지). 비용 캡: DEEP_RESEARCH_DAILY_LIMIT(기본 5·429) + (code,날짜) 공유 캐시 + force 불허 + 종목별 in-flight Mutex + 실패 시 쿼터 반환. 유닛 10 + 실호출 2종목(005930·012450) + 재시작 후 캐시 적중 검증 / [x] C2 클라(Sonnet, 2026-07-11) — 상세 화면 툴바 버튼(iOS=doc.text.magnifyingglass·Android=Search아이콘)+로딩 UX(스피너/LinearProgress)+리포트 카드(청록 테마·핵심 요약 박스·소제목 단락·출처 목록 링크)+자동 펼침, iOS+Android 동시. 빌드 통과.
- [x] D1 실측 (2026-07-11, Fable) — `SensitivityValidationService` + `GET /sensitivity-validation`(관리 라우트, 운영 기능 아님). 지표 이력 2년(Yahoo 5종+ECOS)×바스켓(그룹별 2~5종목 동일가중, DailyHistory 재사용) 페어링. 정렬=지표 시간대별(usdkrw·rate3y 동일일 / 미국 세션 익일), 노이즈 필터=자기 중앙값, 판정=SUPPORTED(일치≥54%&r>0 or t≥2)/CONTRADICTED(일치≤46%&t≤−2 이중)/INCONCLUSIVE(유지). **결과: 지지 6(나스닥 3칸 압도적 61~71%, rate3y 3칸)·반증 5(copper 3칸 전부 — 원가 논리가 경기 선행 신호에 역전 / usdkrw 반도체·자동차 — 수출 채산성이 리스크오프 자금 흐름에 역전, 6/6 그룹 방향 일치)·무근거 15(crude·usdjpy 전부)**. 유닛 16 + 실행 리포트 `docs/sensitivity-validation-2026-07.md`(D2 교정안 포함)
- [x] D2 교정 (2026-07-11, Fable) — copper 3칸 −1→+1, usdkrw 6칸 +1→−1(지표 차원 통일 교정 — 셀 단위 반전은 비일관 테이블이라 기각), note 재작성(환율=리스크오프 신호·구리=경기 선행 신호), decisions.md #14 기록, MacroSignalTest에 교정 방향 고정 테스트 2건(회귀 방지). **실호출 확인: 원화 강세+리스크온 날 구 테이블이면 "수출 채산성 악화=부담" 역해석이 났을 상황을 "외국인 자금 유입=우호"로 정확히 해석, 구리 상승도 경기 회복 신호로 반영**. 기존 방향 의존 테스트 4건 갱신, 전체 테스트 통과
